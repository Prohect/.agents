...
playwright-cli list
# open a headed session (reuse if one already exists)
playwright-cli -s=bg open --headed https://playwright.dev
# navigate to a page
playwright-cli -s=bg goto https://playwright.dev
# interact with the page using refs from the snapshot
playwright-cli -s=bg click e15
playwright-cli -s=bg type "page.click"
...
**Tab hygiene:** Chrome closes the window when the last tab closes. After finishing a task, leave at least one tab open (e.g., `about:blank`) rather than closing all tabs, to keep the window alive:

```bash
playwright-cli -s=bg tab-new about:blank   # keep-alive tab
playwright-cli -s=bg tab-close 1           # close the task tab, not the last one
```

```bash
# Preferred workflow
playwright-cli list                         # check existing sessions
# If no headed session exists:
playwright-cli -s=bg open --headed https://example.com
# [user moves window to another desktop]
# All subsequent commands reuse -s=bg:
playwright-cli -s=bg tab-new https://other.com
playwright-cli -s=bg click e15
playwright-cli -s=bg type "query"
# Never close the -s=bg session
```
...
```bash
playwright-cli list
playwright-cli -s=fg open --headed https://example.com/form
playwright-cli -s=fg snapshot

playwright-cli -s=fg fill e1 "user@example.com"
playwright-cli -s=fg fill e2 "password123"
playwright-cli -s=fg click e3
playwright-cli -s=fg snapshot
```
...
playwright-cli list
playwright-cli -s=bg open --headed https://example.com
playwright-cli -s=bg tab-new https://example.com/other
playwright-cli -s=bg tab-list
playwright-cli -s=bg tab-select 0
playwright-cli -s=bg snapshot
```
...
playwright-cli list
playwright-cli -s=bg open --headed https://example.com
playwright-cli -s=bg click e4
playwright-cli -s=bg fill e7 "test"
playwright-cli -s=bg console
playwright-cli -s=bg requests
```

```bash
playwright-cli -s=bg open --headed https://example.com
playwright-cli -s=bg tracing-start
playwright-cli -s=bg click e4
playwright-cli -s=bg fill e7 "test"
playwright-cli -s=bg tracing-stop
```
...
playwright-cli list
playwright-cli -s=bg open --headed https://example.com
playwright-cli -s=bg show --annotate
```
